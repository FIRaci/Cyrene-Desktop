...
export function detectAssistantOperationalIntent(text: string): boolean {
  ...
  const musicVi = /(ph\u00e1t nh\u1ea1c|b\u1eadt nh\u1ea1c|m\u1edf nh\u1ea1c|nghe nh\u1ea1c|b\u00e0i h\u00e1t|d\u1eebng nh\u1ea1c|t\u1ea1m d\u1eebng nh\u1ea1c|t\u1eaft nh\u1ea1c|chuy\u1ec3n b\u00e0i|next b\u00e0i|b\u00e0i ti\u1ebfp theo)/iu;
...
export async function buildAgentRunOptions(
  ...
  const latestUserText = contentToText(userMessages.at(-1)?.content) ?? "";
  const recentUserContext = userMessages.slice(-2).map((m) => contentToText(m.content)).join(" ");
  const requestedExecutionMode = resolveExecutionMode(
    input.executionMode ?? ((input.style || "").startsWith("talk") ? "chat" : "work"),
  );
  // If user is in "chat" mode but asks for operational secretary/assistant work (e.g. scheduling, reminders, weather, music),
  // automatically promote to "work" mode so Cyrene can execute the required tools instead of mere roleplay yapping!
  const hasOperationalIntent = detectAssistantOperationalIntent(latestUserText) || detectAssistantOperationalIntent(recentUserContext);
